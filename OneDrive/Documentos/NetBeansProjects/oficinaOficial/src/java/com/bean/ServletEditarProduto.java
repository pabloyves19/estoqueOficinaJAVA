...
public class ServletEditarProduto extends HttpServlet {
    ...
    produto.setCategoriaId(categoriaId); // NOVO

    ProdutoDAO produtoDAO = new ProdutoDAO();
    produtoDAO.alterar(produto);

    request.setAttribute("sucesso", "Produto atualizado com sucesso!");
    request.getRequestDispatcher("index.jsp").forward(request, response);
} catch (Exception e) {
    e.printStackTrace();
    request.setAttribute("erro", "Erro ao editar produto: " + e.getMessage());
    request.getRequestDispatcher("index.jsp").forward(request, response);
}
